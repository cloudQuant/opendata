...
# SECURITY: upstream hardcodes a real Xueqiu token here. It MUST NOT be committed.
# Injected from the environment instead; empty value degrades to an unauthenticated
# request (same behaviour as an expired token upstream). Registered as a manual edit
# so the codemod never restores the literal (see THIRD_PARTY_NOTICES.md).
xq_a_token = os.environ.get("XQ_A_TOKEN", "")
